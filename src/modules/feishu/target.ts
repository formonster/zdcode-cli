...
const normalizeTargetValue = (rawTarget: string) => {
  ...
  if (lowered.startsWith('chat:')) {
    return stripPrefix(withoutProvider, 'chat:')
  }
  if (lowered.startsWith('group:')) {
    return stripPrefix(withoutProvider, 'group:')
  }
  if (lowered.startsWith('channel:')) {
    return stripPrefix(withoutProvider, 'channel:')
  }
  if (lowered.startsWith('user:')) {
    return stripPrefix(withoutProvider, 'user:')
  }
  if (lowered.startsWith('dm:')) {
    return stripPrefix(withoutProvider, 'dm:')
  }
  if (lowered.startsWith('open_id:')) {
    return stripPrefix(withoutProvider, 'open_id:')
  }
...
export const resolveFeishuTarget = (
  rawTarget: string,
  forcedReceiveIdType?: FeishuReceiveIdType,
